...
class LatentDirichletAllocation(TopicModel):
    def infer_topics(self, num_topics=10, algorithm='variational', **kwargs):
        ...
        if algorithm == 'variational':
            lda_model = LDA(n_components=num_topics, learning_method='batch', random_state=10,verbose=1)
        ...
        vectorizer = self.corpus.vectorizer
        dic = vectorizer.get_feature_names()
        # Save all data necessary for later prediction
        model = (dic,lda_model.components_,lda_model.exp_dirichlet_component_,lda_model.doc_topic_prior_)
        ...
        for doc in topic_document:
            ...
            for topic_weight in doc:
                ...
                topic_count += 1
            doc_count += 1
        print(self.corpus.size, self.nb_topics)
        print(row)
...
class NonNegativeMatrixFactorization(TopicModel):
    def infer_topics(self, num_topics=10, **kwargs):
        self.nb_topics = num_topics
        nmf = NMF(n_components=num_topics,max_iter=1000, alpha=0.05 , l1_ratio = 0.9, verbose=True, random_state=123)
        #nmf = NMF(n_components=num_topics,random_state=123)
        topic_document = nmf.fit_transform(self.corpus.sklearn_vector_space)
        joblib.dump(nmf, 'nmfModel.pkl')
        self.topic_word_matrix = []
        self.document_topic_matrix = []
        vocabulary_size = len(self.corpus.vocabulary)
        row = []
...
class TruncatedSVD(TopicModel):
    def infer_topics(self, num_topics=10, **kwargs):
        self.nb_topics = num_topics
        nmf = TruncatedSVD(n_components=num_topics)
        topic_document = nmf.fit_transform(self.corpus.sklearn_vector_space)
        joblib.dump(nmf, 'svdModel.pkl')
        self.topic_word_matrix = []
        self.document_topic_matrix = []
        vocabulary_size = len(self.corpus.vocabulary)
        row = []
